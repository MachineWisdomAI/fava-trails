...
caller-controlled metadata and relationships. After Trust Gate review, the
assembled record (including reviewer, reasoning, provider, and model fields) is
scanned again before any governance write. Nested walks that exceed depth 32
fail closed; unscanned content is never treated as clean. It is not complete
DLP, does not rewrite history, and does not delete already-stored records.

## Where candidate content goes

| Step | Stored? | Sent off-box? | Notes |
| --- | --- | --- | --- |
| Tool input (`content`, `reason`, metadata, relationships) | Process memory only until a write succeeds | No | MCP argument logs record lengths, not bodies |
| Preflight reject on save / update / supersede | No new thought file; no new JJ/Git snapshot of the candidate | No | Scans body plus nested caller-controlled strings after hooks; safe error names the pattern id only |
