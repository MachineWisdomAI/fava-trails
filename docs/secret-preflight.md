...
caller-controlled metadata and relationships. It is not complete DLP, does not
rewrite history, and does not delete already-stored records.
...
| `propose_truth` LLM review | Already on disk from the draft | **Yes** — thought body and redacted metadata (`project` / `branch` / `tags`) are in the reviewer user message | Trust Gate reject metadata is written back onto the draft |
