...
**Published 0.7.0:** Versioned thought store with durable persistence before successful return (interruptions can still leave recoverable dirty/incomplete state), governed/authoring/history visibility (process-configured identity; #72/#93), Trust Gate LLM review plus per-call operator `approval="human"` provenance, supersession lineage, MCP integration, lexical `recall`, 1-hop relationship expansion, local Rich Views reader. [Published on PyPI](https://pypi.org/project/fava-trails/0.7.0/) and [GitHub](https://github.com/MachineWisdomAI/fava-trails/releases/tag/v0.7.0) on September 13, 2026. Confirm the loaded runtime with `fava-trails version` (see [runtime-and-upgrade.md](runtime-and-upgrade.md)).

**Older published 0.6.0:** Trust Gate provider-neutral LLM config and related runtime fields shipped; **does not** include the later governed-read isolation / MCP registration fixes that identify as 0.7.0 on this tree. Do not assume 0.6.0 draft/authoring behavior matches this FAQ’s governed visibility model.
