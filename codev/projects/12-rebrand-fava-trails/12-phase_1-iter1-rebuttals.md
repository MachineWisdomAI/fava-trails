...
**Feedback:** README.md JSON examples used `FAVA_TRAILS_DATA_REPO` (old name) while the config table used `FAVA_TRAILS_DATA_REPO` (new name per spec).

**Action taken:** Fixed. All `FAVA_TRAILS_DATA_REPO` occurrences in README.md replaced with `FAVA_TRAILS_DATA_REPO`. Committed in fix commit.

## Intentional Design (no changes)

### FAVA_TRAILS_DIR vs FAVA_TRAILS_SCOPE prefix difference
...
- `FAVA_TRAILS_SCOPE` — singular confirmed by architect: "this is the TRAIL for the particular agent"
No change needed.
