...
Added to spec. `fava-trails init --scope mw/eng/foo` skips interactive prompt. If `--scope` is not provided and `.fava-trails.yaml` is absent, falls back to `input()`.

### 3. Robust `.env` parser (ACCEPTED — all 3 models)
Acknowledged as the highest-risk implementation area. Implementation notes strengthened: parse line-by-line into memory model, preserve comments/blanks/trailing whitespace, handle duplicate `FAVA_TRAILS_SCOPE` entries by updating in-place. Test added for "duplicate key in existing file" scenario.
