...
Added to step 1b: `init` accepts optional `--scope` argument. If provided, skips `input()` prompt. If absent and `.fava-trails.yaml` missing, falls back to interactive prompt.
...
Added to step 1g: `test_init_env_duplicate_key` — verifies that running `init` when `.env` already has duplicate `FAVA_TRAILS_SCOPE` entries results in a single entry (deduplication).
