...
4. **Wired together** — Verified all 36 tests pass with `FAVA_TRAIL_HOME` pointing to `wise-fava-trail/`.
...
| `FAVA_TRAIL_HOME` correctly points server to data repo | Pass |
...
When `FAVA_TRAILS_DIR` points outside `$FAVA_TRAIL_HOME`, the function still creates `$FAVA_TRAIL_HOME/trails` but not the actual configured trails directory.
