...
### Added
- `fava-trails register` prints native MCP registration using an ordinary `FAVA_TRAILS_AGENT_ID`, the resolved executable, and the intended data repository. `--write` is an explicit client-config opt-in (atomic write, `.bak` backup, preserves existing restrictive file modes, new files `0600`, permission denial without bypass). `--verify` labels a direct MCP smoke test, client-config inspection, and a native client session (MCP Inspector loads the config). It does not treat a direct stdio spawn as a native session, and reports `native_client_unavailable`, stale runtime paths, or registration not loaded.
